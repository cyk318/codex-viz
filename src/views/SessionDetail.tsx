...
const tabs = ['timeline', 'tools', 'tokens', 'graph', 'raw'] as const;

export function SessionDetail() {
  const { id } = useParams();
  const [params, setParams] = useSearchParams();
  const active = tabs.includes(params.get('tab') as never) ? params.get('tab')! : 'timeline';
  const { data, error, loading } = useSession(id);

  if (loading) return <main className="mx-auto max-w-[1500px] px-4 py-8 text-sm text-slate-500">Loading session...</main>;
  if (error || !data) return <main className="mx-auto max-w-[1500px] px-4 py-8 text-sm text-red-600">{error || 'Session not found'}</main>;

  return (
    <main className="mx-auto max-w-[1500px] px-4 py-4">
      <Link to="/" className="text-sm text-blue-700 dark:text-blue-300">Back to sessions</Link>
      <div className="mt-3 rounded border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-2 grid gap-2 text-xs text-slate-500 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>{data.turnCount} turns · {data.messageCount} messages · {data.toolCallCount} tools</div>
        ...
        {data.parseWarnings.length ? (
          <div className="mt-3 rounded border border-amber-300 bg-amber-50 p-2 text-xs text-amber-800 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
            {data.parseWarnings.length} parse warnings. The view skipped invalid lines but kept the rest of the session.
          </div>
        ) : null}
      </div>
