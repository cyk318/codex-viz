...
export function SessionList() {
  ...
  const [pricingStatus, setPricingStatus] = useState<string>('built-in pricing');
  ...
  async function refreshPricing() {
    ...
    try {
      ...
      await loadData();
      const suffix = snapshot.warnings?.length ? ` · ${snapshot.warnings.length} warning(s)` : '';
      setPricingStatus(`official pricing ${new Date(snapshot.updatedAt).toLocaleTimeString()}${suffix}`);
    } catch (err) {
      setError((err as Error).message);
      setPricingStatus('pricing refresh failed');
    } finally {
  ...
  return (
    <main className="mx-auto grid max-w-[1500px] gap-4 px-4 py-4 lg:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="rounded border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="border-b border-slate-200 p-3 text-sm font-semibold dark:border-slate-800">Projects</div>
        ...
        <div className="max-h-[calc(100vh-170px)] overflow-auto">
          {projects.map((item) => (
            <button key={item.id} className={`block w-full border-t border-slate-100 px-3 py-2 text-left text-sm dark:border-slate-800 ${project === item.cwd ? 'bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-200' : ''}`} onClick={() => setProject(item.cwd)}>
              ...
              <div className="text-xs text-slate-500">{item.sessionCount} sessions · {formatNumber(item.totalTokens)} tokens</div>
            </button>
          ))}
        </div>
      </aside>
      ...
      <section className="min-w-0">
        <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">{loading ? 'Loading local JSONL files...' : `${filtered.length} sessions`}</p>
          </div>
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
            <button
              className="h-10 rounded border border-slate-300 bg-white px-3 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
              disabled={refreshingPricing}
              onClick={refreshPricing}
              title="Fetch latest pricing from official OpenAI pages and recalculate cost estimates"
            >
            ...
            <input
              ...
              placeholder="Search messages, commands, patches"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
            />
          </div>
        </div>
        <div className="mb-3 text-xs text-slate-500">Cost estimates use {pricingStatus}.</div>
        ...
        {searchResults.length ? (
          <div className="mb-4 rounded border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
            <div className="border-b border-slate-200 p-3 text-sm font-semibold dark:border-slate-800">Search results</div>
        ...
        <div className="overflow-hidden rounded border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
          ...
          {filtered.map((session) => (
            <Link key={session.id} to={`/sessions/${session.id}`} className="grid gap-2 border-b border-slate-100 px-3 py-3 text-sm hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800 lg:grid-cols-[minmax(260px,1fr)_110px_110px_120px_120px] lg:gap-3">
              ...
              <div className="self-center" title={`${formatNumber(sumTokens(session.totalTokens))} tokens, estimated ${formatUsd(session.estimatedCostUsd)}`}>
          ...
          {!loading && filtered.length === 0 ? <div className="p-8 text-center text-sm text-slate-500">No sessions found.</div> : null}
        </div>
      </section>
    </main>
  );
